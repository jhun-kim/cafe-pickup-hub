...
Backend가 없거나 API contract parse가 실패하면 silent success로 숨기지 않고, 화면에 `API 상태: demo fallback`과 이유를 표시합니다. Build는 backend가 없어도 demo fallback으로 완료됩니다. Demo fallback 상태에서는 booking form이 실제 예약 성공처럼 동작하지 않고 생성 버튼이 비활성화됩니다.
